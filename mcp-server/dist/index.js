...
// ---------------------------------------------------------------------------
server.registerTool("get_mission_status", {
    description: "Get the current mission status from the orchestrator. Returns progress, active agents, and next actions.",
...
server.registerTool("validate_project", {
    description: "Validate that a project meets quality standards by checking for required configuration files, test setup, and code quality tools.",
    inputSchema: z.object({
...
// ---------------------------------------------------------------------------
server.registerTool("generate_todo", {
    description: "Generate a TODO.md template for a mission with hierarchical task decomposition.",
    inputSchema: z.object({
...
// ---------------------------------------------------------------------------
server.registerTool("get_agent_roster", {
    description: "Get the full list of available agents with their roles, capabilities, and recommended use cases.",
...
// ---------------------------------------------------------------------------
server.registerTool("context7_resolve_library", {
    description: "Resolve a library name to a Context7-compatible library ID for documentation queries. Use before querying docs.",
    inputSchema: z.object({
        libraryName: z
            .string()
            .describe('Official library name (e.g., "React", "Next.js", "Express")'),
        query: z
            .string()
        ...
        content: [
            {
                type: "text",
                text: JSON.stringify({
                    instruction: "Use the Context7 MCP tools (context7_resolve-library-id then context7_query-docs) to fetch real documentation. This tool provides the lookup strategy.",
                    libraryName,
                    query,
                    recommendedWorkflow: [
...
// ---------------------------------------------------------------------------
server.registerTool("check_crud_completeness", {
    description: "Check CRUD completeness for a module entity. Returns a matrix of required operations and their status.",
    inputSchema: z.object({
...
server.registerTool("create_checkpoint", {
    description: "Create a checkpoint snapshot for compaction recovery. Saves current mission state to survive context window compression.",
    inputSchema: z.object({
...
}, async ({ missionName, progress, activeAgent, currentTask, lastCompletedStep, nextStep, filesModified, blockers, }) => {
    const checkpoint = {
        timestamp: new Date().toISOString(),
        mission: missionName,
        progress,
